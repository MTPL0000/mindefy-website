...
export default function ImprovedCopyPage() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Animation starts when section 6 wrapper reaches the top (like section 1)
      const section6WrapperElement = document.getElementById("section-6-wrapper");
      if (section6WrapperElement) {
        const rect = section6WrapperElement.getBoundingClientRect();
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          minHeight: `calc(100vh + ${Math.max(0, section6Progress) * (typeof window !== 'undefined' ? window.innerHeight : 800) * 0.8}px)`,
          position: 'relative'
        }}
