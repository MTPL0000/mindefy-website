...
export default function ServicesDrop() {
  return (
    <div className="w-full pt-4 sm:pt-6 lg:pt-8 bg-white shadow-lg rounded-[0.5rem] z-20">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 px-4 sm:px-6 lg:px-20 pb-4 sm:pb-6 lg:pb-8">
        {/* Column 1: Modern Application Development + IT/Staff Argumentation */}
        <div className="space-y-4 sm:space-y-6">
          <div>
            <h3 className="text-base sm:text-lg lg:text-[1.25rem] font-semibold text-[#332771] mb-2">
            ...
            <ul className="space-y-1 sm:space-y-1.5 text-[#333333] font-medium text-xs sm:text-sm lg:text-[0.85rem]">
              {groupedServices["Modern Application Development"]?.map((service) => (
                <li key={service.id}>
                  <Link
                    href={service.route}
                    className="hover:text-red-600 hover:font-semibold block py-0.5"
                  >
                    {service.title}
                  </Link>
                </li>
